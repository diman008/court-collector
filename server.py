...
def norm_candidates(text):
    ...
    for p in pats:
        for m in re.finditer(p, text, re.I):
            # Normalize whitespace and append the matched fragment.
            # Using a set avoids duplicates when the same norm is mentioned
            # multiple times in the text with different spacing or casing.
            found.add(safe(m.group()))
    return sorted(found)
